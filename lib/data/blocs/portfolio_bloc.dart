...
class PortfolioBloc extends Bloc<PortfolioEvent, PortfolioState> {
  ...
  PortfolioBloc({required this.portfolio})
      : super(PortfolioInitial(portfolio)) {
  ...
  Future<void> fetchPortfolio(int id) async {
    try {
      final Portfolio p =
          await portfolioService.fetchPortfolio(id, portfolio.period);
      portfolio = p;
  ...
  List<DataPoint> getDataPoints(List<CandleStick> list) {
    return list.map((time) => DataPoint(time.time, 0)).toList();
  }

  void setPeriod(String p) async {
    try {
      final port = await portfolioService.fetchPortfolio(portfolio.id, p);
      final newPort = portfolio.copyWith(period: p, series: port.series);
      portfolio = newPort;
